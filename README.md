...
# What happens when statistics problem simply state a fact ("there is at least one boy"), without stating how this information was obtained?

  What happens when you are just presented with a factual statement like "A parent has two kids, and at least one is a boy", with no information about how this was gathered?  This is common in many problems we are presented with, and we are acting as simply an omniscient observer to the truth.  Most people tend to resort to the polling/survey method that we would use to get the 1/3 or 2/3 answer, but I'd argue this is a mistake.  This method rqquires us to exert a preference/bias/manual selection. A fact about a single instance is actually a purely random disclosure. It doesn't matter if we are speaking about a parent having at least one boy, what numbers are seen in a dice roll, or some other statistical problem that similarly states a singular fact. Since you were told about only one of these instances, in a sea of other instances, this is the definition of a random information disclosure. In our boy/girl paradox, being that this is a random disclosure, the odds would be 50/50 for all possible outcomes.    
